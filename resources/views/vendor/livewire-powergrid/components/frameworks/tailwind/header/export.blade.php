<div x-data="{ open: false }"
    ...
    <button @click.prevent="open = ! open"
            class="block bg-slate-50 text-slate-700 border border-slate-300 rounded m-2 p-2 leading-tight
...
تصدير البيانات
    ...
    <div x-show="open"
        ...
        @if(in_array('excel', data_get($setUp, 'exportable.type')))
            <div class="flex p-2 text-slate-400 dark:text-slate-300">
                <span class="w-12 font-bold">إكسل</span> =>
        ...
        @if(in_array('csv', data_get($setUp, 'exportable.type')))
            <div class="flex p-2 text-slate-400 dark:text-slate-300">
                <span class="w-12 font-bold">CSV</span> =>
