    ...
    @if($column->toggleable['enabled'] && !$showDefaultToggle && $showToggleable === true)
        <div class="flex justify-center">
            <div class="relative rounded-full w-12 h-6 transition duration-200 ease-linear"
                 :class="[toggle === 1 ? 'bg-blue-400 dark:bg-blue-500' : 'bg-blue-100']">
                <label
                    class="absolute  bg-white border-2 mb-2 w-6 h-6 rounded-full  duration-100 ease-linear cursor-pointer"
                    :class="[toggle === 1 ? 'left-0 translate-left-0 border-blue-400 transition transform' : 'right-0 translate-right-0 border-slate-400 transition transform']"></label>
                <input type="checkbox"
